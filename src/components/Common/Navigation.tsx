...
import { Calendar, LayoutDashboard, Users, Clock, PartyPopper, Sunset, Sun, Archive, PiggyBank, Briefcase, ChevronDown, ListCheck, Table, Gift, Circle, Columns, Camera, Armchair } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
interface NavigationProps {
  activeTab: TaskDay | 'overview' | 'timeline' | 'team' | 'calendar' | 'users' | 'cajon-sastre' | 'budget' | 'suppliers' | 'days' | 'guests' | 'tables' | 'gifts' | 'bomboniere' | 'sunday-columns' | 'photo' | 'seating';
  onTabChange: (tab: TaskDay | 'overview' | 'timeline' | 'team' | 'calendar' | 'users' | 'cajon-sastre' | 'budget' | 'suppliers' | 'days' | 'guests' | 'tables' | 'gifts' | 'bomboniere' | 'sunday-columns' | 'photo' | 'seating') => void;
        ...
        <div className="flex items-center gap-1 overflow-x-auto py-2 -mb-px no-scrollbar">
          ...
          {isAdmin && (
            <button
              ...
              title={t('nav.seating', 'Seating Plan')}
              className={cn(
                'flex items-center justify-center px-3 py-3 text-sm font-medium border-b-2 transition-all',
                activeTab === 'seating'
                  ? 'border-primary text-primary'
          ...
          {isAdmin && (
            <button
              ...
              title={t('nav.photo', 'Photo Groups')}
              className={cn(
                'flex items-center justify-center px-3 py-3 text-sm font-medium border-b-2 transition-all',
                activeTab === 'photo'
                  ? 'border-primary text-primary'
